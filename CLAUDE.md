...
### 코드 수정 후 자동 커밋
- 코드 수정 작업이 완료되면 **사용자 확인 없이 바로 커밋** 진행
- 빌드/타입체크 성공 확인 후 커밋
- **커밋 메시지는 한글로 작성** (제목, 본문 모두)
- Conventional Commits 형식 사용: `feat:`, `fix:`, `docs:`, `refactor:` 등
- 예시: `feat: 로그인 기능 추가`, `fix: 계산 오류 수정`

### 대화 로깅
- 중요한 기능 구현이나 학습 포인트가 있는 세션은 `/log-session` 사용
- `/log-session` 실행 시 자동으로 커밋됨
